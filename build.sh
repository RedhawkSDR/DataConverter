...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.DataConverter.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.DataConverter-3.0.1
        tar czf ${tmpdir}/rh.DataConverter-3.0.1.tar.gz --exclude=".svn" -C ${tmpdir} rh.DataConverter-3.0.1
        rpmbuild -ta ${tmpdir}/rh.DataConverter-3.0.1.tar.gz
        rm -rf $tmpdir
    else
