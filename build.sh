...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.DataConverter.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.DataConverter-4.0.3
        tar czf ${tmpdir}/rh.DataConverter-4.0.3.tar.gz --exclude=".svn" -C ${tmpdir} rh.DataConverter-4.0.3
        rpmbuild -ta ${tmpdir}/rh.DataConverter-4.0.3.tar.gz
        rm -rf $tmpdir
    else
